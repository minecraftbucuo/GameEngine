...
# 产物：
#   服务器 build-server/server/GameEngineServer（config.json + maps 已随构建拷入同级 Asset/）
...
build_server() {
    ...
    echo "==> 服务器版完成：build-server/server/GameEngineServer"
    echo "    （config.json 与关卡地图 JSON 已由 CMake 随构建拷入同级 Asset/）"
}
