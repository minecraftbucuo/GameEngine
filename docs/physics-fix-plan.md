...
- [ ] **改动**
  - `src/CollisionSystem.cpp:55-60`：收集完 `collisionPairs` 后，对每个 pair **只发布一个事件**（只发 `"onCollision" + pair.a->getTag()`），不再给 b 也发一份。
  - 这样每个 pair 只有一方（a）执行“分离 + 冲量”，消除双倍修正与双倍能量注入。
  - 注：a 侧已有 `if (!a->getMoveAble() && !b->getMoveAble()) continue;`，但若 a 恰好是静态对象（如 Ground），应把事件发给“可移动的一方”（`b`）；实现上简单处理：若 `!a->getMoveAble()` 则改发 b。
- [ ] **完成标准**：单对球碰撞时，分离距离约为重叠深度的一半（不再是 2 倍）；叠球抖动明显减弱。
