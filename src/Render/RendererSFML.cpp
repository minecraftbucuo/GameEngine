...
// Renderer 的 SFML 实现（回滚路径：CMake 关 BUILD_WITH_SDL3 即回到本实现）。
// Step 10 起 Types.h 为自研类型，与 sf:: 类型间需显式转换（toSf/toSfV）。
...
namespace {
// 引擎单实例窗口（原 Renderer 私有成员，Step 10 起藏进实现文件）
sf::RenderWindow* window = nullptr;

sf::Color toSf(const Color c) { return sf::Color(c.r, c.g, c.b, c.a); }
sf::Vector2f toSfV(const Vec2f v) { return {v.x, v.y}; }
} // namespace
...
Vec2u Renderer::getSize() const {
    return window ? Vec2u(window->getSize().x, window->getSize().y) : Vec2u(0, 0);
}

void Renderer::setSize(const Vec2u size) {
    if (window) window->setSize(sf::Vector2u(size.x, size.y));
}
...
void Renderer::drawRect(const FloatRect& r, const Color fillColor, const bool filled,
    ...
    shape.setFillColor(filled ? toSf(fillColor) : sf::Color::Transparent);
    if (outlineThickness != 0.f) {
        shape.setOutlineColor(toSf(outlineColor));
...
void Renderer::drawLine(const Vec2f a, const Vec2f b, const Color c) {
    if (!window) return;
    const sf::Color col = toSf(c);
    sf::Vertex line[] = {
        sf::Vertex(toSfV(a), col),
        sf::Vertex(toSfV(b), col)
    };
...
void Renderer::drawLines(const std::vector<Vec2f>& points, const Color c) {
    ...
    for (const auto& p : points) {
        vertices.emplace_back(toSfV(p), col);
    }
...
void Renderer::drawPolygon(const std::vector<Vec2f>& points, const Color c) {
    ...
    for (std::size_t i = 0; i < points.size(); ++i) {
        shape.setPoint(i, toSfV(points[i]));
    }
    shape.setFillColor(toSf(c));
    window->draw(shape);
}
...
void Renderer::drawCircle(const Vec2f center, const float radius, const Color c, const bool filled,
    ...
    shape.setFillColor(filled ? toSf(c) : sf::Color::Transparent);
    // 非填充时保证至少有 1px 轮廓可见（与迁移前 SFML 行为一致）
    const float thickness = outlineThickness != 0.f ? outlineThickness : (filled ? 0.f : 1.f);
    if (thickness != 0.f) {
        shape.setOutlineColor(outlineThickness != 0.f ? toSf(outlineColor) : toSf(c));
...
static void fillRoundedRect(sf::RenderWindow* win, const eng::FloatRect& r, const float radius,
    ...
    sf::RectangleShape rects[3] = {
        sf::RectangleShape(sf::Vector2f(w - 2 * rad, h)),
        sf::RectangleShape(sf::Vector2f(rad, h - 2 * rad)),
        sf::RectangleShape(sf::Vector2f(rad, h - 2 * rad)),
    };
    ...
    for (const auto& corner : corners) {
        ...
        va.append(sf::Vertex(sf::Vector2f(corner.center.x, corner.center.y), color));
        for (int i = 0; i <= cornerPoints; ++i) {
            const float angle = corner.a0 + (corner.a1 - corner.a0) * i / cornerPoints;
            va.append(sf::Vertex(
                sf::Vector2f(corner.center.x + std::cos(angle) * rad,
...
void Renderer::drawRoundedRect(const FloatRect& r, const float radius, const Color fillColor,
    ...
    if (outlineThickness > 0.f) {
        fillRoundedRect(window,
                        ...
                        radius + outlineThickness, toSf(outlineColor));
    }
    fillRoundedRect(window, r, radius, toSf(fillColor));
}
...
void Renderer::setCamera(const Vec2f center, const Vec2f size, const float zoom) {
    if (!window) return;
    sf::View view(sf::Vector2f(center.x, center.y), sf::Vector2f(size.x, size.y));
...
Renderer::CameraState Renderer::getCamera() const {
    ...
    return {Vec2f(view.getCenter().x, view.getCenter().y),
            Vec2f(view.getSize().x, view.getSize().y), 1.f};
}
...
Vec2f Renderer::screenToWorld(const Vec2i screenPos) const {
    ...
    const sf::Vector2f w = window->mapPixelToCoords(sf::Vector2i(screenPos.x, screenPos.y));
    return {w.x, w.y};
}
