...
bool loadStaticObjects(Scene& scene, const std::string& json_path = kLevel1MapPath);

// 动态出生点数据读取（单机场景用它生成马里奥/板栗仔/乌龟大王；联机只用玩家出生点）
std::optional<MapDynamicData> loadDynamicData(const std::string& json_path = kLevel1MapPath);
